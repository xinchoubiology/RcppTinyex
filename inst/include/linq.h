...
namespace utils{
  ...
  struct linq_lambda_retriever
  {
    typedef void ResultType;
  };

  template<typename TClass, typename TResult, typename ...TArgs>
  struct linq_lambda_retriever<TResult(__thiscall TClass::*)(TArgs...)>
  {
    typedef TResult ResultType;
  };

  template<typename TClass, typename TResult, typename ...TArgs>
  struct linq_lambda_retriever<TResult(__thiscall TClass::*)(TArgs...)const>
  {
    typedef TResult ResultType;
  };
  ...
  struct zip_pair
  {
    ...
    bool operator==(const zip_pair<T, U>& p){ return p.first == first && p.second == second; }
    bool operator!=(const zip_pair<T, U>& p){ return p.first != first || p.second != second; }
  };
  ...
  namespace iterators{
    ...
    public:
      storage_iterator(const std::shared_ptr<std::vector<T>>& _values,
                       const typename std::vector<T>::iterator& _iterator) : values(_values), iterator(_iterator){}
    ...
    public:
      ...
      T operator*()const
      {
        throw linq_exception("Failer to get a value from an empty collection");
      }
  ...
  namespace types{
    ...
    template<typename TIterator, typename TFunction>
    using take_where_it = iterators::take_while_iterator<TIterator, TFunction>;
  ...
  public:
    ...
    linq_enumerable(const TIterator& __begin, const TIterator& __end)
...
#define SUPPORT_STL_CONTAINERS_EX(NAME, TYPES, PARAMETERS, ARGUMENTS)                                                       \
    ...
    ///////////////////////////////////////////
    // iterating (lazy evaluation) constructor
    ///////////////////////////////////////////
    ...
    linq_enumerable<types::skip_while_it<TIterator, TFunction>> skip_while(const TFunction& f)const
    {
      return linq_enumerable<types::skip_while_it<TIterator, TFunction>>(
          ...
          types::skip_while_it<TIterator, TFunction>(_end, end, f)
      );
    }
    ...
    TElement last()const
    {
      if (empty()) throw linq_exception("Failer to get a value from an empty collection.");
      auto it = _begin;
      TElement result = *it;
      while (++it != end)
      {
    ...
    linq_enumerable<TIterator> single()const
    {
      ...
      it++;
      if (it != _end) throw linq_exception("The collection shoule not have exactly one value.");
    ...
    linq<TElement> single_or_default(const TElement& value)const
    {
      ...
      it++;
      if (it != _end) throw  linq_exception("The collection should not have exactly one value.");
    ...
    linq<TElement> distinct()const
    {
      ...
      auto xs = std::make_share<std::vector<TElement>>();
      for (auto it = _begin; it != _end; it++)
      {
    ...
    linq<TElement> except_with_(const linq_enumerable<TIterator2>& e)const
    {
      ...
      // from value used to translate different data structure to linq-class
      return from_values(xs);
    }
    SUPPORTS_STL_CONTAINERS(except_with)
    ...
    {
      auto it = _begin;
      if (it == _end) throw linq_exception("Failed to get a avalue from an empty collection.");
...
#else
          ->linq<decltype(*((typename linq_lambda_retriever<decltype(&TFunction::operator())>::ResultType*)0)->begin())>
#endif
         {
    ...
    auto group_by(const TFunction& keySelector)const->linq<zip_pair<decltype(keySelector(*(TElement*)0)), linq<TElement>>>
    {
      ...
      typedef std::share_ptr<TValueVector>           TValueVectorPtr;
      ...
      for (auto it = _begin; it != _end; it++)
      {
        ...
        // if key is not been found in map
        if (it2 == map.end())
        {
          ...
          >>
    {
      ...
      auto result = std::make_shared<std::vector<TValue1>>();
    ...
    SUPPORT_STL_CONTAINERS_EX(
      full_join,
      PROJECT_PARAMETERS(typename TFunction1, typename TFunction2),
      PROJECT_PARAMETERS(const TFunction1& keySelector1, const TFunction2& keySelector2),
      PROJECT_PARAMETERS(keySelector1, keySelector2)
    )
